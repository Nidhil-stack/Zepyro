...
.. class:: BMP180(i2cdrv, addr=0x77, clk=100000)
    ...
    def __init__(self, drvname, addr=LCD_I2CADDR, clk=400000):
        i2c.I2c.__init__(self, addr, drvname, clk)                                      # Initialize I2C
        self._addr = addr
        self.init()


    def init(self):                                                                     # Initialize display
        """
        .. method:: init()
    ...
    def _command(self, cmd):                    # Send command to LCD
        self._send(cmd, 0)
    ...
    def _send(self, data, mode):                # Send data to LCD
        highBits = data & 0xF0
        lowBits = (data << 4) & 0xF0
        self._write4bits(highBits | mode)
        self._write4bits(lowBits | mode)

    def _write4bits(self, data):                # Send a 4 bits command to LCD followed by backlight data
